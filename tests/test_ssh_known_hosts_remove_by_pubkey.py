...
def test_ssh_known_hosts_edit_remove_by_pubkey_1a(known_hosts_filename):
    # no file
    assert Path(known_hosts_filename).exists() is False
    edit = SSHKnownHostsEdit(known_hosts_file_location=known_hosts_filename)
    assert Path(known_hosts_filename).exists() is False
    assert edit.remove_from_known_hosts_by_pubkey(TEST_KEY_1) is False
    assert Path(known_hosts_filename).exists() is False


def test_ssh_known_hosts_edit_remove_by_pubkey_1b(known_hosts_filename):
    # empty file
...
def test_ssh_known_hosts_edit_remove_by_pubkey_1c(known_hosts_filename):
    # file with existing other content
    _write_file_content(known_hosts_filename, f"{TEST_HOSTNAME} {TEST_KEY_1}\n")
    edit = SSHKnownHostsEdit(known_hosts_file_location=known_hosts_filename)
    assert edit.remove_from_known_hosts_by_pubkey(TEST_KEY_2) is False
    _assert_file_content(
        known_hosts_filename,
...
def test_ssh_known_hosts_edit_remove_by_pubkey_1d(known_hosts_filename):
...
def test_ssh_known_hosts_edit_remove_by_pubkey_1e(known_hosts_filename):
...
def test_ssh_known_hosts_edit_remove_by_pubkey_1h(known_hosts_filename):
